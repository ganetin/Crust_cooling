...
c       idel2=int(Log10(rhodrip/rhosurf)*float(idec+iref))
c	endif
...
c          write(98,*)jj,
c     1     rad_t(j),bar_t(j),rho_t(j),pres_t(j),emas_t(j),phi_t(j)
...
c if rhosurf < rhoenv, determines ienv. 
c else ienv=isurf+2
...
c       INCLUDE 'files_phys.inc.f'
       ...
       INCLUDE 'profile_star.inc.f'
       INCLUDE 'profile_comp.inc.f'
       dimension rho_t(1000),pres_t(1000),bar_t(1000),
     1           A_cell_t(1000),A_ion_t(1000),Z_ion_t(1000)
...
c          A_cell(i)=w1*A_cell_t(j)+w2*A_cell_t(j+1)
c          A_ion(i) =A_ion_t(j+1)
c          Z_ion(i) =Z_ion_t(j+1)
c FORTIN-04/2010 New new version: A, Z, A_cell constant:
          A_cell(i)=A_cell_t(j+1)
          A_ion(i) =A_ion_t(j+1)
          Z_ion(i) =Z_ion_t(j+1)
c FORTIN 10/2010 : all quantities are interpolated
c          A_cell(i)=w1*A_cell_t(j)+w2*A_cell_t(j+1)
c          A_ion(i) =w1*A_ion_t(j) +w2*A_ion_t(j+1)
c          Z_ion(i) =w1*Z_ion_t(j) +w2*Z_ion_t(j+1)
         end if
...
c        do j=0,imax
c          write(97,*)rrho(j),yneutr(j)*bar(j)
c        end do
...
c 20-05-2010: Effective mass from SLy4 Skyrme interaction
        else if (emncr.eq.10.) then
         do i=icore+1,idrip
           rhon=kfn(i)**3./(3.*pi**2)
           htm=hbc**2/939.0
           mstn(i) = 1./(1.+2.*(57.1287+24.6567)/htm*rhon)
         end do
c 20-05-2010 - end
        end if
c If version = NEW exit because core mst's already gotten from EOS table
!! COMMENTED IN DEC 2015 FOR OFENGEIM
c        if ((version.eq.'NEW').or.(version.eq.'QRK')) return
!!
c Core neutrons:
        if (emnco.lt.1.) then
         do i=0,icore
...
c 20-05-2010: Effective mass from SLy4 Skyrme interaction
         ...
         do i=0,icore
		   ...
		   yi=(rhon-rhop)/rho
           htm=hbc**2./939.0
           mstn(i) = 1./(1.+2.*(57.1287+yi*24.6567)/htm*rho)
         end do
c 20-05-2010 - end
        end if
c Protons:
...
c 20-05-2010: Effective mass from SLy4 Skyrme interaction
         ...
         do i=0,icore
		   ...
		   yi=(rhon-rhop)/rho
           htm=hbc**2./939.0
           mstp(i) = 1./(1.+2.*(57.1287-yi*24.6567)/htm*rho)
         end do
c 20-05-2010 - end
        end if
c Hyperons:
...
c *********************************************************************
       ...
       INCLUDE 'quark.inc.f'
        do j=0,imax
         ...
         mn =939.56d0*mstn(j)
cc BSK21
         mn =939.565379d0*mstn(j)
cc
         pfp=kfp(j)*197.d0
cc BSK21
         mp =938.27d0*mstp(j)
         mp =938.272046d0*mstp(j)
cc
         pfla=kfla(j)*197.d0
...
c*Declaration of the variables for the HFB calculation
	  character*100 f_HFB
      dimension HFB_rho(1:30),HFB_dens_n(1:30),
     1              HFB_d0_d(1:30),HFB_a0_d(1:30),
     2              HFB_a1_d(1:30),HFB_a2_d(1:30),
     3              HFB_a3_d(1:30)
      dimension log_a0(0:isize),log_a1(0:isize),
     1          log_a2(0:isize),log_a3(0:isize),
     2          log_d0(0:isize)
...
c      Gap obtained with Skyrme interaction for weak and strong pairing
c      see M. Fortin et al., PRC xxx (2010)
      else if (sfn1s0.eq.6.) then
       do i=0,idrip
        tcn(i)=max(1.d0,tcn1_gsknw(kfn(i)))*fn1s0
       end do
      else if (sfn1s0.eq.7.) then
       do i=0,idrip
        tcn(i)=max(1.d0,tcn1_gskns(kfn(i)))*fn1s0
       end do
      else if (sfn1s0.eq.8.) then
       do i=0,idrip
        tcn(i)=max(1.d0,tcn1_eehjo96(kfn(i)))*fn1s0
       end do
c     Ioffe gaps:
       ...
       else if ((sfn1s0.eq.301.) .or. (sfn1s0.eq.302.)) then
c***********************************************************
c**** HFB interpolations
          kboltz=1.380d-16
          MeV2erg=1.602177d-6

c READ HFB PARAMETERS: **********************************
      if (sfn1s0.eq.301.) then
          f_HFB=  'I_Files/I_HFB_param_NV_W.dat'
      else if (sfn1s0.eq.302.) then
          f_HFB=  'I_Files/I_HFB_param_NV_S.dat'
      endif
c* HFB CALCULATIONS 
       write(6,*)f_HFB(1:istrlen(f_HFB))
c* ***
      if (debug.ge.1.) print *,'Opening I_HFB*.dat'
      open(unit=20,file=f_HFB,status='old')
      read(20,*)
      read(20,*)i_HFB_max       
      do i=1,i_HFB_max
          read(20,*)i_HFB_zone_number,HFB_rho(i),HFB_dens_n(i),
     1              HFB_d0_d(i),HFB_a0_d(i),
     2              HFB_a1_d(i),HFB_a2_d(i),
     3              HFB_a3_d(i)
      enddo
      close(unit=20,status='keep')
       ...
       do i=icore,idrip
          dens=rrho(i)
          j=1
          do while ((dens-HFB_rho(j)).lt.0.d0) 
               j=j+1
          enddo

          dh=( dlog10( dens ) - dlog10( HFB_rho(j) ) )
     1       /( dlog10( HFB_rho(j-1) ) - dlog10( HFB_rho(j) ) )

          log_a0(i)=(1.d0-dh)*dlog10( HFB_a0_d(j) )
     1               +dh*dlog10( HFB_a0_d(j-1) )
          log_a1(i)=(1.d0-dh)*dlog10( HFB_a1_d(j) )
     1               +dh*dlog10( HFB_a1_d(j-1) )
          log_a2(i)=(1.d0-dh)*dlog10( HFB_a2_d(j) )
     1               +dh*dlog10( HFB_a2_d(j-1) )
          log_a3(i)=(1.d0-dh)*dlog10( HFB_a3_d(j) )
     1               +dh*dlog10( HFB_a3_d(j-1) )
          log_d0(i)=(1.d0-dh)*dlog10( HFB_d0_d(j) )
     1               +dh*dlog10( HFB_d0_d(j-1) )
          hfb_a0(i)=10**log_a0(i)
          hfb_a1(i)=10**log_a1(i)
          hfb_a2(i)=10**log_a2(i)
          hfb_a3(i)=10**log_a3(i)
          if (hfb_a3(i) .lt. 1.d-5) then
               hfb_a3(i)= 1.d-5 
          endif
          hfb_d0(i)=10**log_d0(i)
          if (hfb_d0(i).lt.1.d-5) hfb_d0(i)=1.d-5 
          tcn(i)=hfb_a0(i)*hfb_d0(i)*MeV2erg/kboltz
      enddo 
c Corresponds to Shternin et al. (2007) with moderate superfluidity       
      else if (sfn1s0.eq.401.) then
       do i=0,idrip
        tcn(i)=max(1.d0,tcn1_wap(kfn(i)))*fn1s0
       end do
c************************************************************
