      subroutine specheat_hfb (i,t,rho,aion,zion,cv,
     1       cvneutron,cvproton,cvelectron,cvmuon,
     2       cvlambda,cvsminus,cvszero,cvsplus,
     3       cvquark,cvions,
     4       hfb_a0i,hfb_a1i,hfb_a2i,hfb_a3i,hfb_d0i,
     5       hfb_tcni)
...
c      f_1(t,tcni,a0i,a1i,a3i,d0i)=(1.d0+dexp(-a0i*a1i*d0i/a3i))/
c     1        (1.d0+dexp((t*tcni/8.61734315d11-a0i*a1i*d0i)/a3i))
c      f_2(t,tcni,a0i,a2i,a3i,d0i)=(1.d0+dexp(-a0i*a2i*d0i/a3i))/
c     1        (1.d0+dexp((t*tcni/8.61734315d11-a0i*a2i*d0i)/a3i))
c      h(u,t,tcni,a0i,a1i,a2i,a3i,d0i)=
c     1           r_1s0(u)*f_1(t,tcni,a0i,a1i,a3i,d0i)
c     2         +( 1.d0-f_2(t,tcni,a0i,a2i,a3i,d0i) )


      f_1(t,a0i,a1i,a3i,d0i)=(1.d0+dexp(-a0i*a1i*d0i/a3i))/
     1        (1.d0+dexp((t/(MeV/kb)-a0i*a1i*d0i)/a3i))
      f_2(t,a0i,a2i,a3i,d0i)=(1.d0+dexp(-a0i*a2i*d0i/a3i))/
     1        (1.d0+dexp((t/(MeV/kb)-a0i*a2i*d0i)/a3i))
      h(u,t,a0i,a1i,a2i,a3i,d0i)=
     1           r_1s0(u)*f_1(t,a0i,a1i,a3i,d0i)
     2         +( 1.d0-f_2(t,a0i,a2i,a3i,d0i) )


c      f_1(t,a0i,a1i,a3i,d0i)=(1.d0+dexp(-a0i*a1i*d0i/a3i))/
c     1        (1.d0+dexp((t/8.61734315d11-a0i*a1i*d0i)/a3i))
c      f_2(t,a0i,a2i,a3i,d0i)=(1.d0+dexp(-a0i*a2i*d0i/a3i))/
c     1        (1.d0+dexp((t/8.61734315d11-a0i*a2i*d0i)/a3i))
...
c      if ((rho.ge.rhodrip).and.(istrange.eq.0)) then
c HERE DANY cccccccccccccccccccccccc
c       raise=1.1d0
       raise=1.0d0
cccccccccccccccccccccccccccccccccccc
c       if (t .lt. raise*hfb_tcni)then
c        t0=min(0.999999999999d0,t/hfb_tcni)
c        if (i.le.isf) then
c         u=u_3p2(t0)
c         r=r_3p2(u)
c        else
c         u=u_1s0(t0)
c         r=r_1s0(u)
c         write(*,*)'1s0 superfluidity'
c        end if
c        if (t .gt. hfb_tcni) then
c         w1=(raise*hfb_tcni-t)/((raise-1.)*hfb_tcni)
c         w2=1.-w1
c         r=w1*r+w2*1.0
c        end if
c       else
...
c         u=u_3p2(t0)
c         r=r_3p2(u)
c         write(84,*)'3p2 superfluidity',rho
c        else
c         u=u_1s0(t0)
c         r=r_1s0(u)
c         hfb_f1=f_1(t0,hfb_tcni,hfb_a0i,hfb_a1i,hfb_a3i,hfb_d0i)
c         hfb_f2=f_1(t0,hfb_tcni,hfb_a0i,hfb_a2i,hfb_a3i,hfb_d0i)
c         hfb_h=r*hfb_f1+(1-hfb_f2)
c          write(88,'(11E14.4)')rho,t0,hfb_tcni,hfb_a0i,hfb_a1i,
c     1     hfb_a2i,
c     1    hfb_a3i,hfb_d0i,hfb_f1,hfb_f2,hfb_h

c         write(84,*)'1s0 superfluidity',rho
c        end if
c        if (t .gt. hfb_tcni) then
c         w1=(raise*hfb_tcni-t)/((raise-1.)*hfb_tcni)
c         w2=1.-w1
c         r=w1*r+w2*1.0
c         write(85,*)t,hfb_tcni,r
c        end if
c       else
c       hfb_h=1.d0
c       end if
c***
          ...
          write(84,'(7E14.4)')rho,hfb_a0i,hfb_a1i,
     4      hfb_a2i,hfb_a3i,hfb_d0i,hfb_tcni
...
c** tcni
     ...
     1 then
          write(*,*)rho,'in the inner-crust',hfb_tcni
            if (t .gt. 20*hfb_tcni) then
               r=0.d0  
               else if (t .gt. hfb_tcni) then
                    u=0.001
                    r=r_1s0(u)
                    else
c                    t0=min(0.999999999999d0,t/hfb_tcni)
...
c          write(83,'(7E14.4)')rho,hfb_a0i,hfb_a1i,
c     4      hfb_a2i,hfb_a3i,hfb_d0i,hfb_tcni
c             hfb_f1=f_1(t,hfb_a0i,hfb_a1i,hfb_a3i,hfb_d0i)
           hfb_f1=(1.d0+dexp(-hfb_a0i*hfb_a1i*hfb_d0i/hfb_a3i))/
     1     (1.d0+dexp((t/(MeV/kb)-hfb_a0i*hfb_a1i*hfb_d0i)/hfb_a3i))
c             hfb_f2=f_1(t,hfb_a0i,hfb_a2i,hfb_a3i,hfb_d0i)
         hfb_f2=(1.d0+dexp(-hfb_a0i*hfb_a2i*hfb_d0i/hfb_a3i))/
     1      (1.d0+dexp((t/(MeV/kb)-hfb_a0i*hfb_a2i*hfb_d0i)/hfb_a3i))
             hfb_h=r*hfb_f1+(1-hfb_f2)
             cvneutron =cvn(i)*hfb_h*t
          write(88,'(7E14.4)')rho,t0,hfb_tcni,t,
     1          hfb_f1,hfb_f2,hfb_h
          write(*,*)'our HFB calculations'
      else if ((rho.lt.rhodrip).and.(istrange.eq.0)) then
          write(*,*)rho,'in the outer-crust'
             cvneutron=0.d0     
      else if ((rho.gt.rhocore).and.(istrange.eq.0)) then
          write(*,*)rho,'in the core'
            if (t .lt. hfb_tcni)then
             t0=min(0.999999999999d0,t/hfb_tcni)
                  if (i.le.isf) then
                   u=u_3p2(t0)
                   r=r_3p2(u)
                  else
            ...
            cvneutron =cvn(i)*t*r
          write(*,*)'D. Page calculations'
        end if



c       if (rho.lt.rhocore) then
c           cvneutron =cvn(i)*hfb_h
c       else
c            cvneutron=0.d0
c       end if
c     endif
...
cc 12/02/2010      cvneutron =cvn(i)*t*r
c       if (rho.lt.rhocore) then
cc       cvneutron =cvn(i)*t*h(u,t,tcn(i),hfb_a0i,hfb_a1i,
cc     1            hfb_a2i,hfb_a3i,hfb_d0i)

cc******************
c       cvneutron =cvn(i)*hfb_h
c          write(90,'(4E14.4)')rho,hfb_h,cvn(i),cvneutron
cc******************
cc       cvneutron =cvn(i)*t*h(u,t0,hfb_tcni,hfb_a0i,hfb_a1i,
cc     1            hfb_a2i,hfb_a3i,hfb_d0i)

cc       write(*,*)'t  ;  tcni  ;  t0'
cc       write(*,'(3E14.4)')t,hfb_tcni,t0
cc*** control of the calculation of the specific heats ***c
cc       write(87,'(5E14.4)')rrho(i),
cc     1   h(u,t0,hfb_tcni,hfb_a0i,hfb_a1i,hfb_a2i,hfb_a3i,hfb_d0i),
cc     2   r_1s0(u),
cc     3 f_1(t0,hfb_tcni,hfb_a0i,hfb_a1i,hfb_a3i,hfb_d0i),
cc     4   f_2(t0,hfb_tcni,hfb_a0i,hfb_a2i,hfb_a3i,hfb_d0i)
cc       write(86,'(12E14.4)')rrho(i),
cc     5 hfb_tcni,hfb_a0i,hfb_a1i,hfb_a2i,hfb_a3i,hfb_d0i,
cc     6 -hfb_a0i*hfb_a1i*hfb_d0i/hfb_a3i,
cc     7 -hfb_a0i*hfb_a2i*hfb_d0i/hfb_a3i,
cc     8 t0*hfb_tcni,
cc     9 t0,hfb_tcni
...
cccccccccccccccccccccccccccccccccccc
       ...
       cvneutron =cvn(i)*t*r
          write(87,'(6E14.4)')rho,t0,tcn(i),t,
     1          u,r
      else
       cvneutron=0.d0
      end if
