...
c *                                                                   *  
c *  From Jaikumar et al.  	Phys.Rev. D72 (2005) 123004                                            *
c *                                                                   *
...
c *  of a neutron star from electron-nuclei bremsstrahlung.           *
c *                                                                   *  
...
c*********************************************************************
      subroutine durca_crust(i,t,qdurca_crust)
c
c *****************************************************************
c           DURCA shells in the crust
c *****************************************************************
       implicit real*8 (a-h,k-z)
       dimension q_durca(14),mue_durca(14)
       INCLUDE 'size.inc.f'
       INCLUDE 'profile_star.inc.f'
       INCLUDE 'fermi.inc.f'
       dimension qdurca(0:isize)
       pi = 3.14159265

       data mue_durca/07.6,08.3,10.3,10.7,11.2,11.7,
     1  11.8,12.1,13.3,13.3,13.4,13.8,14.3,14.7/

       data q_durca  /01.41,02.56,00.96,02.56,01.47,00.74,14.09,
     1  17.62,01.04,38.44,13.29,05.61,01.04,01.55/
c *****
        do k=icore,0,-2
         qdurca(k)=0
         qdurca(k-1)=0.
        enddo
         ...
         do j=1,14
          if ( (mue_durca(j).ge.muei+t/1.1604d10) .and.
     1         (mue_durca(j).lt.mueim2-t/1.1604d10) ) then
           qdurca(i)=qdurca(i)+q_durca(j)*1e22
        if (qdurca(i).ne.0.) then        
         muemin=mue_durca(j)-t/1.1604d10
         muemax=mue_durca(j)+t/1.1604d10
         rmax=rad(i-2)+(rad(i)-rad(i-2))/(muei-mueim2)*(muemax-mueim2)
         rmin=rad(i-2)+(rad(i)-rad(i-2))/(muei-mueim2)*(muemin-mueim2)
         dvoldc=4.*pi*((rmax+rmin)/2.)**2*
     x                   (rmin-rmax)
         if (dvoldc.lt.0.) dvoldc=-1.*dvoldc
        endif
          end if
         end do

         qdurca_crust=qdurca(i)*(t/1e9)**5./(dvol(i)+dvol(i+1))*dvoldc
c       if (qdurca_crust.ne.0.d0) then
c       write(90,*)i,t,muei,rrho(i),qdurca_crust
c       write(90,*)i,t,muei,rrho(i),1e6
c       write(90,*)
c       endif
       return
      end
