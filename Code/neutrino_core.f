...
c Murca_n:  n+n -> n+p+e+nu:
      ...
      alpha_n =1.13!1.76d0-0.63d0*(1.68d0/kfn(i))**2
      beta_n  =0.68d0
      qmurca_ne=8.55e21 *rmn**3*rmp   * 
     1         (yprot(i)*bar(i)/0.16d0)**(1.d0/3.d0) *
...
!     2         alpha_p * beta_p * (t/1.d9)**8
      if    ( kfn(i) .lt.3.*kfp(i) +kfe(i) ) then
      qmurca_pe=8.55e21 *rmn**3*rmp   * 
     1         (yprot(i)*bar(i)/0.16d0)**(1.d0/3.d0) *
     2         alpha_n * beta_n * (t/1.d9)**8 *(rmp/rmn)**2.*
     2         (kfe(i)+3.*kfp(i)-kfn(i))**2./(8.*kfe(i)*kfp(i))

      endif


! DO NOT FORGET THE MUONS!!
c Murca_n:  n+n -> n+p+mu+nu:
      if (ymuon(i).ne.0.d0) then
      alpha_n =1.13!1.76d0-0.63d0*(1.68d0/kfn(i))**2
      beta_n  =0.68d0
      qmurca_nm=8.55e21 *rmn**3*rmp   * 
     1         (yprot(i)*bar(i)/0.16d0)**(1.d0/3.d0) *
     2         alpha_n * beta_n * (t/1.d9)**8 
     3         *(ymuon(i)/yelect(i))**(1.d0/3.d0) 
c Murca_p:  n+p -> p+p+mu+nu: 
      if    ( kfn(i) .lt.3.*kfp(i) +kfm(i) ) then
      qmurca_pm=8.55e21 *rmn**3*rmp   * 
     1         (yprot(i)*bar(i)/0.16d0)**(1.d0/3.d0) *
     2         alpha_n * beta_n *(rmp/rmn)**2.* (t/1.d9)**8 *
     2         (kfm(i)+3.*kfp(i)-kfn(i))**2./(8.*kfm(i)*kfp(i))
     3         *(ymuon(i)/yelect(i))**(1.d0/3.d0)
      else
      qmurca_pm=0.d0
      endif
...
c **** effect of superfluidity :
      ...
      qmurca_n=rmurca_n*(qmurca_ne+qmurca_nm)
      qmurca_p=rmurca_p*(qmurca_pe+qmurca_pm)
      qmurca_nucl=qmurca_p+qmurca_n
      return
...
c**********************************************************************
c Includes the Levenfish & Yakovlev suppression factors
c**********************************************************************
...
c alpha_nn for the core:
c      alpha_nn =0.59d0                ! THIS IS TOO HIGH FOR THE CRUST
c alpha_nn for the crust:
      up=140.d0/(2.d0*kfn(i)*197.3d0)  ! This is m_pi/p_F
      if (kfn(i).lt.1.d-10) then
       alpha_nn=0.d0
      else
       alpha_nn =1.d0-1.5d0*up*datan(1.d0/up)+up**2/2.d0/(1.d0+up**2)
      end if
      beta_nn  =0.56d0
...
c      take out the fraction of volume occupied by nuclei:
c FORTIN Jan 2016
c      qbrem_nn=rbrem_nn*qbrem_nn * (1.d0-vion)
...
cc**********************************************************************
c
c      subroutine nubrem_crust_nn(i,t,vion,qbrem_nn)
c
cc **** checked partially on March 30, 1993
c
cc**********************************************************************
cc Includes the Levenfish & Yakovlev suppression factors for DURCA,
cc and only Boltzmann factors for MURCA.
cc**********************************************************************
c
c      implicit real*8 (a-h,k-z)
c      INCLUDE 'size.inc.f'
c      INCLUDE 'rho_limits.inc.f'
c      parameter (pi = 3.14159265d0)
c        
c      INCLUDE 'profile_star.inc.f'
c      INCLUDE 'profile_comp.inc.f'
c      INCLUDE 'pairing.inc.f'
c      INCLUDE 'fermi.inc.f'
c      INCLUDE 'control_nu.inc.f'
cc ***************************
c      fexp(x)=dexp(max(x,-7.d2))
cc ***************************
cc SINGLET PAIRING:
c      u_1s0(t)=dsqrt(1.d0-t)*(1.456d0-0.157d0/dsqrt(t)+1.764d0/t)
cc Brem_nn:  n+n -> n+n+2nu
c      rbrem_nn_n1s0(u)=
c     1     (0.1747d0+dsqrt((0.8253d0)**2+(.07933d0*u)**2))**2*
c     2     fexp(4.228d0-dsqrt((4.228d0)**2+(4.d0*u)**2))/2.0d0 +
c     3     (0.7333d0+dsqrt((0.2667d0)**2+(0.1678d0*u)**2))**7.5d0*
c     4     fexp(7.762d0-dsqrt((7.762d0)**2+(9.d0*u)**2))/2.0d0
cc ***************************
cc TRIPLET PAIRING:
c      u_3p2B(t)=dsqrt(1.d0-t)*(0.7893d0+1.188d0/t)
cc Brem_nn:  n+n -> n+n+2nu
c      rbrem_nn_n3p2B(u)=rbrem_nn_n1s0(u)
cc **********************************************
c      n_nu=3.d0    ! Number of neutrino famillies !
cc Brem_nn:  n+n -> n+n+2nu
c      alpha_nn =0.59d0
c      beta_nn  =0.56d0
c      qbrem_nn=n_nu * 7.4d19 *       mstn(i)**4     * 
c     1         (kfn(i)/1.68d0) * alpha_nn * beta_nn * (t/1.d9)**8
cc **** effect of superfluidity :
c      if(t .lt. tcn(i))then
c       if (i.ge.isf) then
c        tt=t/tcn(i)
c        u=u_1s0(tt)
c        rbrem_nn=rbrem_nn_n1s0(u)
c       else
c        tt=t/tcn(i)
c        u=u_3p2B(tt)
c        rbrem_nn=rbrem_nn_n3p2B(u)
c       end if
c      else
c       rbrem_nn=1.0d0
c      end if
cc **** Reduction from ion's volume:
c      
c      qbrem_nn=rbrem_nn*qbrem_nn * (1.d0-vion)
c
c      return
...
c **** n-p:
! 4.24 changed into 4.00
      if     (idurca_np(i).eq.1)then
       qdurca_np= rate_np * 4.00d27*rmn*rmp*(t/1.d9)**6*
...
c      write(95,*)rho,qdurca_np,idurca_np(i),r_np,rmn,rmp
...
c ordered in increasing values, for the Slpine interpolation
     ...
     x       status='old')
         ...
         read(33,*)(lgtau2(i),i=35,1,-1)
          write(95,*)'SF_suppression.dat opening'
         do i=35,1,-1
