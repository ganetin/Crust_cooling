...
c *********************************************************************
     ...
     1          w2*dlog(sign_l* teffective)
        ...
        write(10,514)' Time=',tprint(itprint-1),
     1               ' Te_inf=',t_effective
c        write(90,514)' Time=',tprint(itprint-1),
c     1               ' Te_inf=',t_effective
514     format(2(5x,1a7,1p1e15.7))
        write(10,*)
        write(10,516)
     1       ' zone ',
...
cccc
         logtempb=w1*log(otemp(i-2))+w2*log(temp(i-2))
         temperatureb=dexp(logtempb)
cccc
         lumino=0.0d0
         if (i-1.ne.0) then
...
c     4        lambda1(i)*(temperature-temperatureb)/(rad(i)-rad(i-2)),
c     5        pres(i)
c         write(90,888)rad(i)/1.d2/1d3,temperature
        end do
        write(10,*)
c        write(90,*)
c517     format(
...
c 601    format(i8,1p1e12.3,5x,1p4e12.3)
 601    format(i8,1p1g16.9,5x,1p4g16.9)
c*********************************************************************
       else
...
c *********************************************************************
     ...
     1   ( abs(otemp(imax)-temp(imax)).le.5e-10 ) ) then
      write(29,602)m_dot0,(time+dtime)/year,teffective,
     1             lum(imax-1)*lsol,!or is it /ephi(imax-1)**2,
     2             lnu*lsol,
     3             lh*lsol
 602    format(1p1g14.6,1p1g16.9,1p4g16.9)
      endif
