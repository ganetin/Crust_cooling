...
c       eta_0=eta
       ...
       read(20,*)f_TbTs
      close(unit=20,status='keep')
c      if ((ifteff.eq.15) .or. (ifteff.eq.16)) then
...
c READ NEUTRINO PARAMETERS: ********************************************
      ...
      open(unit=20,file=f_Neutrino,status='old')
       ...
       read(20,*)inu_photo
       read(20,*)inu_pair
       read(20,*)inu_durca_crust
       read(20,*)inu_crust       ! Not used so far
...
cc
	...
	i_acc_max=0
         ...
         print '(3a12)',
     1      'm_dot_max','t_out','t_end'
         print '(3a12)',
     1      '[Mo/yr] ','[yr] ','[yr] '

	do i=1,100
        read(20,*,err=68,end=68)m_dot0i
        read(20,*,err=68,end=68)t_acc0i
        read(20,*,err=68,end=68)t_acc2i
	read(20,*)
         print '(1p2e12.3)',
     1      m_dot0i,t_acc0i,
     1   t_acc2i
	m_dot0_tab(i)=m_dot0i
        t_acc0_tab(i)=t_acc0i*3.15576d7
        t_acc2_tab(i)=t_acc2i*3.15576d7
	i_acc_max=i_acc_max+1
       end do
68     continue
       else
      ...
      if (i_acc.eq.3) then
       ...
       mdot_tab(:)=0
       do i=1,i_modelmax
        read(20,*,err=67,end=67)mdot_tab(i)
       end do
67     continue
      end if
	endif
      close(unit=20,status='keep')
      etabound=max(1.d-50,etabound)
c      eta0=max(1.d-50,eta0)

c READ STRANGE QUARK MATTER PARAMETERS: ***************************
      if (istrange.eq.1) then
       if (debug.ge.1.) print *,'Opening I_Strange*.dat'
