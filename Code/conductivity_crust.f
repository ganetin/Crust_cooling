...
c ****************
cc FORTIN 26/11/2010
	if (icon_crust.ne.4) then
 652        if (rho.ge.6.d7) then
c ***** CRUST REGIME:
...
c icon_crust=2:
     ...
     2                              nu_e_s_imp,nu_e_l_imp) 
           ...
           sigma =1.d0/(1.d0/sigma_ph+1.d0/sigma_imp)
           lambda=1.d0/(1.d0/lambda_ph+1.d0/lambda_imp)
          else if ( (icon_crust.eq.3) .or. (icon_crust2.eq.3) ) then
c icon_crust=3:
...
c **********************************************************************
c FORTIN 26/11/2010
	  else if (icon_crust.eq.4) then
		if ( (rho.lt.1e9).and.(T.lt.1e9)) then
cc Routine from Ioffe Institute		
		Zion=Z
		TLG=dlog10(T)
		RLG=dlog10(rho)
...
cc if the routine is not applicable
c ***** CRUST REGIME:
         gamma=2.273e5*Z**2*(rho/A)**(1./3.)/T
         if (gamma .gt. gammacryst) then
           call con_e_phon_ion_GYP(T,rho,A,A1,Z,ifs,
...
c ****************
	icon_crust2=0
cc reset to 0 the flag from icon_crust if out of the range of use.
c **********************************************************************
       return
      end
c *********************************************************************
...
c *********************************************************************
c *********************************************************************
c * From http://www.ioffe.ru/astro/conduct/index.html                 *
c *********************************************************************
c * Calculates the conductivity of stellar plasmas                    *
c * for rho from 10^-6 to 10^9 g/cc                                   *
c * for T from 10^3 to 10^9 K                                         *
c 								      *
c * interpolation of the thermal conductivity from table "condall06.d"*
c * Author: Alexander Potekhin                   Last change: 13.06.06*
...
c * Electron-electron scattering is included according to             * 
c *    A.Y.Potekhin, 2006 (unpublished note).                         *
c * Please quote these publication when using this program.	      *
c * SOURCE DATA FILE ''condall06.d'' MUST BE IN YOUR'		      *
c * CURRENT DIRECTORY						      *
c **********************************************************************
