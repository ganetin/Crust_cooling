...
1230   format(10x,2(1a15,1p1e10.3))
     ...
     2     '  pexo=',  pexo
       write(19,*)
 9750  format(a8,i4,a10,0p1f4.0,8x,  5x    ,a10,0p1f3.0,a10,1p1e12.3)
 9751  format(a8,i4,a10,0p1f4.0,a8,0p1f05.2,a10,0p1f3.0,a10,1p1e12.3)
 9752  format(a8,i4,a10,0p1f4.0,8x,  5x    ,a10,0p1f3.0,a10,0p1f05.0)
 9753  format(a8,e12.3)
 ...
 9792  format(a30,1p1e12.3,a8)
       ...
       if (ifteff.eq.0) then
        ...
        etaboundp=etabound
        if (etabound.le.1.d-40) etaboundp=0.0
        write(19,'(a58,1p1e8.2)')
     2    'Accreted envelope from Potekhin et al. with Eta = ',etaboundp
       else if (ifteff.eq.6) then
        write(19,'(a40)')' No envelope '
...
c       else if (ifteff.eq.16) then
c        write(29,'(a40,1p1e10.1)')
c     2    'Fixed outer boundary temperature T_b during accretion =',tb_acc0   
       end if
       write(19,*)
c ***********************************************************************
       write(19,*)
...
c ***********************************************************************
c **** OPEN the file for the steady state accretion : *******************
c ***********************************************************************
      if (i_acc.eq.3) then
      if (i_model.eq.1) then
      if (debug.ge.1.) print *,'Opening Stead file'
       rstar=rad(imax)
       mstar=emas(imax)
       rdurca=-1.d0
       mdurca=-1.d0
     ...
     1           root/g/emas(idrip)/msol
       ...
       open(unit=29,file=f_stestao,status='new')

       write(29,*)
       write(29,*)model
       write(29,*)
       write(29,1230)'Mstar     =',    mstar,'Rstar     =',    rstar
       if(rdurca.gt.0.)then
        write(29,1230)'Mdurca    =',   mdurca,'Rdurca    =',   rdurca
       end if
       if(jexo.gt.-1)then
        write(29,1230)'Mexo      =',     mexo,'Rexo      =',     rexo
       end if
       write(29,1230)'Mcore     =',    mcore,'Rcore     =',    rcore
       write(29,1230)'Mincrust  =', mincrust,'Rincrust  =', rincrust
       write(29,1230)'Moutcrust =',moutcrust,'Routcrust =',routcrust
       write(29,*)
c1230   format(10x,2(1a15,1p1e10.3))
     ...
     2     '  pexo=',  pexo
       write(29,*)
c 9750  format(a8,i4,a10,0p1f4.0,8x,  5x    ,a10,0p1f3.0,a10,1p1e12.3)
c 9751  format(a8,i4,a10,0p1f4.0,a8,0p1f05.2,a10,0p1f3.0,a10,1p1e12.3)
c 9752  format(a8,i4,a10,0p1f4.0,8x,  5x    ,a10,0p1f3.0,a10,0p1f05.0)
c 9753  format(a8,e12.3)

       if (rdurca.le.0.)then
          write(29,9791)'DUrca not possible'
       else
          if (inu_durca.eq.0) then
             write(29,9791)'DUrca possible but TURNED OFF'
          else 
             write(29,9792)'DUrca active above Rho =',rhodurca,
     2                     'gm/cm^3'
          end if
       end if
       write(29,9790)'e-ion bremstrahlung:',inu_eion,
     2                  'bubble neutrinos:',inu_bubble
       write(29,9790)'plasma neutrinos:',inu_plasma,
     2               ' neutron 1S0 pbf:',inu_n1s0_pbf
       write(29,9790)'  pair neutrinos:',inu_pair,
     2               ' neutron 3P2 pbf:',inu_n3p2_pbf
       write(29,9790)' photo neutrinos:',inu_photo,
     2               '  proton 1S0 pbf:',inu_p_pbf
       write(29,*)
c 9790  format (2(1a30,1i3))
c 9791  format(a30)
c 9792  format(a30,1p1e12.3,a8)

       if (ifteff.eq.0) then
        write(29,'(a30,a40)')'Envelope from file: ',f_TbTs
       else if (ifteff.eq.1) then
        write(29,'(a40)')' Iron envelope from Gundmundsson et al'
       else if (ifteff.eq.2) then
        write(29,'(a40)')' Iron envelope from Nomoto & Tsuruta'
       else if (ifteff.eq.3) then
        etaboundp=etabound
        if (etabound.le.1.d-40) etaboundp=0.0
        write(29,'(a58,1p1e8.2)')
     2    'Accreted envelope from Potekhin et al. with Eta = ',etaboundp
       else if (ifteff.eq.2) then
        write(29,'(a40)')' Envelope model from Hernquist & Applegate'
       else if (ifteff.eq.10) then
        write(29,'(a40,1p1e10.1,a3)')
     2    ' Magnetized Iron envelope with',bf_r(imax),' G'
       else if (ifteff.eq.11) then
        write(29,'(a40,1p1e10.1,a3)')
     2    ' Magnetized Accreted envelope with',bf_r(imax),' G'
       else if (ifteff.eq.15) then
        write(29,'(a40,1p1e10.1)')
     2    'Fixed outer boundary temperature T_b =',tb_acc0   
c       else if (ifteff.eq.16) then
c        write(29,'(a55,1p1e10.1)')
c     2    'Fixed outer boundary temperature T_b during accretion ='
c     3  	,tb_acc0       
       end if
       if (ilgrav.ne.0) then
        write(29,'(a40)')
     2    'Grav. luminosity from infalling particles included'   
       end if
       write(29,*)
c ***********************************************************************
       write(29,*)
c       if (pteff.eq.1.0) then
        write(29,1231)
     1   'mdot',' Time  ',' Teff   ',
     2   ' L_phot  ',' L_nu   ',' L_heat '
        write(29,1231)
