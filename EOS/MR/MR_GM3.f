     ...
     x delta,stepi,barynum
      real*8 mass,massold
      logical output, stndrd, qseek,init,prod
...
ccccccccccccccccccccccccccccccccccccccccccccccc
        emax = barynum(imax)-target
ccccccccccccccccccccccccccccccccccccccccccccccc
        accuracy=1.e-7
        if(.not.qseek(emax,rhoc,emold1,rhoold1,emold2,rhoold2,
     1                accuracy,iii-1))  go to 12

      elseif (prod) then
      if(em(imax).le.1d5) then
      write(7,'(1p5e17.9)') rhoc,r(imax),em(imax),baryden(imax)
     1 ,barynum(imax)
      mass=em(imax)
c      write(90,*)massold,mass
      if (mass.ge.massold)       massold=mass
      else
c      write(6,*)'Houston we have a problem',rhoc,em(imax)
      endif
        if((iii.le.ipmax)) go to 11
!.and.(emold.le.em(imax))
        close(unit=7,status='keep')
      endif
        write(*,*)'Maximum grav. mass (Msol) :',massold
      end
...
c      write(6,*) ' Profile being output. '
